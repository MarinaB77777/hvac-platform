...
export default function MyOrders() {
  ...
  const uploadResult = async (id, url) => {
    const token = localStorage.getItem("token");
    await axios.post(`http://localhost:8000/orders/${id}/upload-result`, { url }, {
      headers: { Authorization: `Bearer ${token}` }
    });
    loadOrders();
  };
  ...
  const uploadDiagnostic = async (id, url) => {
    const token = localStorage.getItem("token");
    await axios.post(`http://localhost:8000/orders/${id}/upload-diagnostic`, { url }, {
      headers: { Authorization: `Bearer ${token}` }
    });
    loadOrders();
  };
  ...
  const rateOrder = async (id, rating) => {
    const token = localStorage.getItem("token");
    await axios.post(`http://localhost:8000/orders/${id}/rate`, { rating: Number(rating) }, {
      headers: { Authorization: `Bearer ${token}` }
    });
    loadOrders();
  };
      ...
      {orders.map(order => (
        <div key={order.id} className="border p-4 mb-2">
          ...
          {order.status === "accepted" && (
            <button onClick={() => updateStatus(order.id, "in_progress")} className="bg-yellow-500 text-white px-3 py-1 mt-2">
              В работу
            </button>
          )}
          {order.status === "in_progress" && (
            <button onClick={() => updateStatus(order.id, "completed")} className="bg-green-600 text-white px-3 py-1 mt-2">
              Завершить
            </button>
          )}
        </div>
